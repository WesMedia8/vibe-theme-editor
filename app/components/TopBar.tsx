  ...
  return (<>
    <header style={{ height: 'var(--topbar-height)', background: 'var(--bg-surface)', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', padding: '0 16px', gap: 16, flexShrink: 0, position: 'relative', zIndex: 50 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: aiProvider === 'openai' ? '#10a37f' : 'var(--cyan)' }} />{aiProvider === 'openai' ? 'OpenAI' : 'Anthropic'}</div>
    ...
    {showSettings && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => e.target === e.currentTarget && setShowSettings(false)}>
      <div className="animate-fade-in" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', width: 440, padding: '24px' }}>
        ...
        <div style={{ marginBottom: 16 }}><label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', letterSpacing: '0.08em', marginBottom: 6, textTransform: 'uppercase' }}>AI Provider</label><div style={{ display: 'flex', gap: 8 }}>{PROVIDER_OPTIONS.map(opt => (<button key={opt.value} onClick={() => { setSettingsProvider(opt.value); setNewKey(''); setKeySaved(false) }} style={{ flex: 1, padding: '8px 12px', background: settingsProvider === opt.value ? 'var(--bg-overlay)' : 'var(--bg-surface)', border: `1px solid ${settingsProvider === opt.value ? 'var(--cyan)' : 'var(--border-subtle)'}`, borderRadius: 'var(--radius-md)', cursor: 'pointer', fontSize: 12, fontWeight: settingsProvider === opt.value ? 600 : 400, color: settingsProvider === opt.value ? 'var(--text-primary)' : 'var(--text-secondary)', textAlign: 'center' }}>{opt.label}</button>))}</div></div>
