...
export default function EditorView({
  ...
  // Shared ref for the editor window — persists across Code/Preview switches
  const editorWindowRef = useRef<Window | null>(null)

  // Build the Shopify theme editor URL
  ...
  // Auto-open the Shopify theme editor when store + theme are ready
  useEffect(() => {
    if (!editorUrl) return
    // Only auto-open if no editor window is already open
  ...
  // Keep the editor open — if user closes it, reopen after a short delay
  useEffect(() => {
    if (!editorUrl) return
    ...
    <div style={{
      ...
      <div style={{
        ...
        {viewMode === 'code' ? (
          ...
          /* Preview mode: editor launcher + status */
          <PreviewPanel
            shopDomain={state.shopDomain}
            selectedTheme={state.selectedTheme}
